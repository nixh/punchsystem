    ...
    peroids.forEach(function(peroid, index){
        ...
        record.forEach(function(userRec){
            ...
            userReport.regularPayments = isOvertime ? 
                                     overtimeSetting.threshold * userReport.rate :
                                     userReport.totalHours * userReport.rate;
            userReport.overtimePayments = isOvertime 
                                         ? userReport.overtimeHours 
                                            * userReport.rate 
                                            * overtimeSetting.multiplier
                                         : 0
            userReport.totalPayments = userReport.regularPayments + userReport.overtimePayments;
...
function weeklyDetailsToOverall(reports) {
    ...
    reports.forEach(function(report, index){
        ...
        for(var userid in report.reports) {
            ...
            if(!overallUserReport.userid) overallUserReport.userid = userid;

            if(!overallUserReport.userName)
                overallUserReport.userName = currentReport.userName;

            if(!overallUserReport.from)
                overallUserReport.from = currentReport.from;
            else if(overallUserReport.from > currentReport.from)
                overallUserReport.from = currentReport.from;

            if(!overallUserReport.to)
                overallUserReport.to = currentReport.to;
            else if(overallUserReport.to < currentReport.to)
                overallUserReport.to = currentReport.to;
            
            if(!overallUserReport.rate)
                overallUserReport.rate = currentReport.rate;

            if(!overallUserReport.overtimeHours)
                overallUserReport.overtimeHours = currentReport.overtimeHours;
            else 
                overallUserReport.overtimeHours += currentReport.overtimeHours;

            if(!overallUserReport.totalHours)
                overallUserReport.totalHours = currentReport.totalHours;
            else
                overallUserReport.totalHours += currentReport.totalHours;

            if(!overallUserReport.overtimePayments)
                overallUserReport.overtimePayments = currentReport.overtimePayments;
            else 
                overallUserReport.overtimePayments += currentReport.overtimePayments;

            if(!overallUserReport.totalPayments)
                overallUserReport.totalPayments = currentReport.totalPayments;
            else
                overallUserReport.totalPayments += currentReport.totalPayments;
